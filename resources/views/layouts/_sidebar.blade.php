...
>
    ...
    <div class="border-b border-slate-100 px-4 py-4">
        <div class="flex items-center gap-3">
            ...
            <div x-show="sidebarOpen" x-transition class="min-w-0">
                ...
                <p class="truncate text-xs capitalize text-slate-400">{{ str_replace('_', ' ', $dashboardRole) }}</p>
            </div>
        </div>
    </div>
        ...
        @if($dashboardRole === \App\Support\Roles::ADMIN_GENERAL)
            ...
            @foreach([['Reportes','chart-bar'],['Configuración','cog']] as $extra)
                <a href="#" class="group mx-3 mb-1 flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-600 transition-all hover:bg-indigo-50 hover:text-indigo-700">
                    ...
                    @endif
                    <span x-show="sidebarOpen" x-transition>{{ $extra[0] }}</span>
    ...
    <div class="mt-auto border-t border-slate-100 p-4">
        <a href="#" class="group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-500 transition hover:bg-indigo-50 hover:text-indigo-700">
            <svg class="h-5 w-5 flex-shrink-0 transition-transform group-hover:scale-110" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M8.23 8.23a5 5 0 117.54 7.54M9.88 9.88a3 3 0 114.24 4.24M12 18h.01M12 6h.01"/>
            </svg>
            <span x-show="sidebarOpen">¿Necesitas ayuda?</span>
        </a>
        <p x-show="sidebarOpen" class="mt-2 px-3 text-[10px] text-slate-300">v1.0.0</p>
    </div>
</aside>
