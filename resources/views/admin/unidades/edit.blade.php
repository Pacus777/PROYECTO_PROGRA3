...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.unidades.update', $unidad) }}" class="max-w-2xl rounded-2xl bg-white p-6 shadow-sm md:p-8">
        ...
        <div class="space-y-5">
            ...
            <div>
                <label for="codigo_ued" class="block text-xs font-semibold text-slate-600 uppercase tracking-wide mb-1.5">Código</label>
            ...
            <div>
                ...
                <textarea name="direccion_ued" id="direccion_ued" rows="3"
                          class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-800 transition focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-300">{{ old('direccion_ued', $unidad->direccion_ued) }}</textarea>
