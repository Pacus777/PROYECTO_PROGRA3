...
@section('content')
    <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500">Gestión de cuentas, roles y datos de persona.</p>
        </div>
        <a href="{{ route('admin.usuarios.create') }}" class="inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 px-5 py-2.5 text-sm font-semibold text-white shadow-md transition hover:from-indigo-700 hover:to-purple-700">
            ...
            <table class="w-full text-sm">
                <thead class="border-b border-slate-100 bg-slate-50 text-left">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-xs font-semibold uppercase tracking-wide text-slate-400">Rol</th>
                        <th class="px-6 py-4 text-xs font-semibold uppercase tracking-wide text-slate-400">Unidad</th>
                ...
                <tbody>
                    @forelse($usuarios as $usuario)
                        ...
                        <tr class="border-b border-slate-50 transition hover:bg-indigo-50/30 last:border-0">
                            ...
                            <td class="px-6 py-4 text-slate-600">{{ $usuario->rol->nombre_rol ?? '—' }}</td>
                            <td class="px-6 py-4 text-slate-600">{{ $usuario->unidadEducativa->nombre_ued ?? '—' }}</td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-right whitespace-nowrap">
                                ...
                                <form method="POST" action="{{ route('admin.usuarios.destroy', $usuario) }}" class="inline" onsubmit="return confirm('¿Eliminar este usuario? Esta acción no se puede deshacer.');">
                                    @csrf
                                    @method('DELETE')
